function [zDot, e_prev,sigma] = human_machine_unsafe(t,z)
global e_previous sigma_previous j level W_stack W_optimal W_stack_unsafe W_optimal_unsafe
...
tao = 1;
if j == 0
    mu2a=-0.5.*(R2\G2_')*phi_p'*W_stack_unsafe(level*2+2,:)';
%     mu1a = 0;
%     if t < t0
...
if j == 1
    mu1a=-0.5.*(R1\G1_')*phi_p'*W_stack_unsafe(level*2+3,:)';
%     mu2a = 0;
%     if t < t0
